...
<?php
	...
	if(isset($_POST['signed_request']))
	{
		...
		{
			try {
				$sql = "SELECT `id`, `user`, `email`, `tokendate` FROM `accountlist` WHERE `instagram`=?";
				...
				if($row !== NULL)
				{
					...
					if(delete($dbh, $row['id'], $row['user'], $row['email'], $row['tokendate'], "Instagram App"))
						echo json_encode(array('success'=>true, 'url'=>HTTP."instagram/deletion/".hash("crc32", $row['id']), 'confirmation_code'=>hash("crc32", $row['id'])));
					else {
						echo json_encode(array('success'=>false, 'error'=>"Unable to log data deletion"));
					}
				}
				else
				{
